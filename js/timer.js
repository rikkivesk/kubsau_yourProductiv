...
function tick(){
    ...
    if (sec == 0 & min == 0 & hrs == 0){
        min = 0; sec = 2; hrs = 0 // первый помидор окончен
        pomidor = pomidor + 1
        pomidoroCount.textContent = "Отдых"
        console.log(pomidor);
        if (pomidor == 2){
            pomidoroCount.textContent = "Второй помидор"
            min = 0; sec = 5; hrs = 0 // второй помидор начат
        }
        if (pomidor == 3){
            min = 0; sec = 2; hrs = 0 // второй помидор закончен
        }
        if (pomidor == 4){
            pomidoroCount.textContent = "Третий помидор"
            min = 0; sec = 5; hrs = 0 // третий помидор начат
        }
        if (pomidor == 5){
            min = 0; sec = 2; hrs = 0 // третий помидор окончен
        }
        if (pomidor == 6){
            pomidoroCount.textContent = "Четвертый помидор"
            min = 0; sec = 5; hrs = 0 // четвертый помидор начат
        }
        if (pomidor == 7){
